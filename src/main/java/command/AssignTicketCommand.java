...
public class AssignTicketCommand extends Command {
    ...
    AssignTicketCommand(final CommandInput input) {
    ...
    public final void execute(final Application app,
        ...
        if (user.getExpertiseArea() != assignedTicket.getExpertiseArea()) {
            if (user.getExpertiseArea() == ExpertiseArea.FRONTEND) {
                Command error = new ErrorCommand(getCommand(), getUsername(),
                        getTimestamp(), "Developer "
                        + getUsername() + " cannot assign ticket "
                        + ticketID
                        + " due to expertise area. Required: BACKEND, DB, FULLSTACK"
            ...
            if (user.getExpertiseArea() == ExpertiseArea.BACKEND) {
                Command error = new ErrorCommand(getCommand(), getUsername(),
                        getTimestamp(), "Developer " + getUsername()
                        + " cannot assign ticket " + ticketID
                        + " due to expertise area. Required: FRONTEND, DEVOPS, DESIGN, FULLSTACK"
                        + "; Current: " + user.getExpertiseArea() + ".");
                commands.add(error);
                return;
            }
            if (user.getExpertiseArea() == ExpertiseArea.DEVOPS) {
                Command error = new ErrorCommand(getCommand(), getUsername(),
                        ...
                        + " FRONTEND, BACKEND, DESIGN, DB, FULLSTACK"
                        + "; Current: " + user.getExpertiseArea() + ".");
                commands.add(error);
                return;
            }
            if (user.getExpertiseArea() == ExpertiseArea.DESIGN) {
                Command error = new ErrorCommand(getCommand(), getUsername(),
                        getTimestamp(), "Developer " + getUsername()
                        + " cannot assign ticket " + ticketID
                        + " due to expertise area. Required: BACKEND, DEVOPS, DB, FULLSTACK"
                        + "; Current: " + user.getExpertiseArea() + ".");
                commands.add(error);
                return;
            }
            if (user.getExpertiseArea() == ExpertiseArea.DB) {
                Command error = new ErrorCommand(getCommand(), getUsername(),
                        getTimestamp(), "Developer " + getUsername()
                        + " cannot assign ticket " + ticketID
                        + " due to expertise area. Required: FRONTEND"
                        + ", BACKEND, DEVOPS, DESIGN, FULLSTACK"
                        + "; Current: " + user.getExpertiseArea() + ".");
                commands.add(error);
                return;
            }
        ...
        if (user.getSeniority() == Seniority.JUNIOR) {
            if (!assignedTicket.getType().equals("FEATURE_REQUEST")) {
                Command error = new ErrorCommand(getCommand(),  getUsername(),
                        getTimestamp(), "Developer " + getUsername()
                        + " cannot assign ticket " + ticketID
                        + " due to seniority level. Required: MID, SENIOR"
            ...
            if (assignedTicket.getBusinessPriority().equals("CRITICAL")) {
                Command error = new ErrorCommand(getCommand(),  getUsername(),
                        getTimestamp(), "Developer "
                        + getUsername() + " cannot assign ticket "
                        + ticketID + " due to seniority level. Required: SENIOR"
                        + "; Current: " +  user.getSeniority() + ".");
                commands.add(error);
                return;
            }
        ...
        if (user.getSeniority() == Seniority.MID) {
            if (assignedTicket.getBusinessPriority().equals("CRITICAL")) {
                Command error = new ErrorCommand(getCommand(),  getUsername(),
                        getTimestamp(), "Developer " + getUsername()
                        + " cannot assign ticket " + ticketID
                        + " due to seniority level. Required: SENIOR"
                        + "; Current: " +  user.getSeniority() + ".");
                commands.add(error);
                return;
            }
        ...
        if (!assignedTicket.getStatus().equals("OPEN")) {
            Command error = new ErrorCommand(getCommand(),
                    getUsername(), getTimestamp(),
                    "Only OPEN tickets can be assigned.");
            commands.add(error);
            return;
        }
        Milestone milestoneOfTheTicket = MilestoneStorage
                .getMilestoneByName(MilestoneStorage
                        .nameOfTheMilestoneThatContaintsTicket(assignedTicket
                                .getId()));
            ...
            if (ok == 0) {
                Command error = new ErrorCommand(getCommand(),
                        ...
                        "Developer " + getUsername()
                                ...
                                + milestoneOfTheTicket.getName() + ".");
                commands.add(error);
                return;
            }
            if (milestoneOfTheTicket.getIsBlocked()) {
                Command error = new ErrorCommand(getCommand(),
                        getUsername(), getTimestamp(), "Cannot assign ticket "
        ...
        ticket.setAssignedAt(app.getCurrentDate().toString());
        Action historyAction = new Assigned("ASSIGNED", getUsername(), getTimestamp());
        ticket.getHistory().add(historyAction);
        historyAction = new StatusChanged("STATUS_CHANGED", getUsername(),
                getTimestamp(), "OPEN", "IN_PROGRESS");
        ticket.getHistory().add(historyAction);
